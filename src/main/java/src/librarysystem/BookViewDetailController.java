    ...
    submitBorrow.setOnAction(event -> {
      ...
      if (confirm) {
        ...
        sendNotification(1000, MainGUI.currentUser.getId(),
            "The request to borrow the book has been submitted. Please wait for confirmation from admin",
            0);
...
//            // Gắn sự kiện click vào từng ngôi sao
  ...
  public void removeBook(ActionEvent actionEvent) {
    ...
    if (DBInfo.isBorrowed(currentBook.getTitle())) {
      sendNotification(1000, MainGUI.currentUser.getId(),
          "Cannot delete books that are being borrowed by users", 1);
      return;
    }
    GaussianBlur blurEffect = new GaussianBlur(10);
    containerPane.setEffect(blurEffect); // Làm mờ phần nội dung chính
  ...
  public void addBook(ActionEvent actionEvent) {
    ...
    sendNotification(1000, MainGUI.currentUser.getId(), "Successfully add a book to the library!!!",
        0);
