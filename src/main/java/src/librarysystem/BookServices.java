...
public class BookServices {
    ...
    if (items != null) {
      for (int i = 0; i < items.size(); i++) {
        ...
        String publishedDate =
            volumeInfo.has("publishedDate") ? volumeInfo.get("publishedDate").getAsString()
                : "Unknown";
        String publisher =
            volumeInfo.has("publisher") ? volumeInfo.get("publisher").getAsString() : "Unknown";

        String numPage =
            volumeInfo.has("pageCount") ? String.valueOf(volumeInfo.get("pageCount").getAsInt())
                : "Unknown";
        ...
        String description =
            volumeInfo.has("description") ? volumeInfo.get("description").getAsString()
                : "No Description";
        String ISBN = "None";
        ...
        String buyLink =
            volumeInfo.has("infoLink") ? volumeInfo.get("infoLink").getAsString() : "Unknown";

        books.add(new Book(title, ISBN, authors, publisher, publishedDate, description, thumbnail,
            numPage, category, price, language, buyLink));
      }
    }
    return books;
  }
  ...
  public ArrayList<Book> searchBooksByAuthor(String query) {
    ...
    try {
      ...
      String apiUrl =
          "https://www.googleapis.com/books/v1/volumes?q=" + formattedQuery + "&key=" + API_KEY;
  ...
  public ArrayList<Book> searchBooksByCategory(String query) {
    ...
    try {
      ...
      String apiUrl =
          "https://www.googleapis.com/books/v1/volumes?q=" + formattedQuery + "&maxResults=20&key="
              + API_KEY;
