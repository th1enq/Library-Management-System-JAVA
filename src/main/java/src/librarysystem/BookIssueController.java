...
public class BookIssueController extends BaseController {
  ...
  private final ArrayList<Book> topList = DBInfo.getBookListByNumView();
  ...
  /**
   * handle the selection user
   *
   * @param list           book list
   * @param selectedButton button
   * @param mode           filterMode
   */
  private void handleCategorySelectionUser(ArrayList<BookIssue> list, Button selectedButton,
      int mode) {
  ...
  private void resetCategoryButtonStylesUser() {
    viewAllUser.setStyle(
        "-fx-background-color: transparent; -fx-background-radius: 5px; -fx-cursor: hand;");
    viewBorrowedUser.setStyle(
        "-fx-background-color: transparent; -fx-background-radius: 5px; -fx-cursor: hand;");
    viewPendingUser.setStyle(
        "-fx-background-color: transparent; -fx-background-radius: 5px; -fx-cursor: hand;");
    viewReturnedUser.setStyle(
        "-fx-background-color: transparent; -fx-background-radius: 5px; -fx-cursor: hand;");
  }
  ...
  private void initForUser() {
    ...
    // Set button actions
    viewAllUser.setOnAction(event -> handleCategorySelectionUser(
        BookIssueDB.getTotalListByUserId(MainGUI.currentUser.getId()), viewAllUser, 0));
    viewBorrowedUser.setOnAction(event -> handleCategorySelectionUser(
        BookIssueDB.getBorrowedListByUserId(MainGUI.currentUser.getId()), viewBorrowedUser, 1));
    viewReturnedUser.setOnAction(event -> handleCategorySelectionUser(
        BookIssueDB.getReturnedListByUserId(MainGUI.currentUser.getId()), viewReturnedUser, 2));
    viewPendingUser.setOnAction(event -> handleCategorySelectionUser(
        BookIssueDB.getPendingListByUserId(MainGUI.currentUser.getId()), viewPendingUser, 4));

    sortTitleUser.setOnAction(event -> {
      resetSortOrderExcept("Title");
      if (isTitleAscending) {
        bookIssueList.sort(
            Comparator.comparing(BookIssue::getBookTitle, String.CASE_INSENSITIVE_ORDER));
      } else {
        bookIssueList.sort(
            Comparator.comparing(BookIssue::getBookTitle, String.CASE_INSENSITIVE_ORDER)
                .reversed());
    ...
    sortAuthorUser.setOnAction(event -> {
      ...
      if (isAuthorAscending) {
        bookIssueList.sort(
            Comparator.comparing(BookIssue::getBookAuthor, String.CASE_INSENSITIVE_ORDER));
      } else {
        bookIssueList.sort(
            Comparator.comparing(BookIssue::getBookAuthor, String.CASE_INSENSITIVE_ORDER)
                .reversed());
  ...
  private void initForAdmin() {
    ...
    viewBorrowed.setOnAction(
        event -> handleCategorySelection(BookIssueDB.getBorrowedList(), viewBorrowed, 1));
    viewReturned.setOnAction(
        event -> handleCategorySelection(BookIssueDB.getReturnedList(), viewReturned, 2));
    viewDelay.setOnAction(
        event -> handleCategorySelection(BookIssueDB.getDelayList(), viewDelay, 3));
    viewPending.setOnAction(
        event -> handleCategorySelection(BookIssueDB.getPendingList(), viewPending, 4));
    ...
    sortUserName.setOnAction(event -> {
      ...
      if (isUserNameAscending) {
        bookIssueList.sort(
            Comparator.comparing(BookIssue::getUsername, String.CASE_INSENSITIVE_ORDER));
      } else {
        bookIssueList.sort(
            Comparator.comparing(BookIssue::getUsername, String.CASE_INSENSITIVE_ORDER).reversed());
      }
  ...
  /**
   * create the String of star
   *
   * @param rating rate
   * @return String of star unicode
   */
  private String getRatingStars(int rating) {
    ...
    return "\uf005".repeat(Math.max(0, rating)) + // Unicode for filled star
  ...
  private Pane createBookPane1() {
    ...
    detailsButton.setStyle(
        "-fx-background-color: #000; -fx-background-radius: 20px; -fx-cursor: hand;");
    ...
    // Rating stars
    String ratingStars = getRatingStars(firstBook.getRating()); // Pass rating and max stars (5)
    ...
    starLabel.setStyle(
        "-fx-font-family: 'FontAwesome'; -fx-text-fill: #FFD700; -fx-font-size: 20px;");
    ...
    pane1.getChildren()
        .addAll(imageView, titleLabel, authorLabel, detailsButton, viewsLabel, ratingLabel,
            categoryLabel, starLabel, eyeIcon);
  ...
  private Pane createBookPane2() {
    ...
    // Icon đánh giá sao
    String ratingStars = getRatingStars(secondBook.getRating());
    Label starLabel = new Label(ratingStars);
    starLabel.setLayoutX(150.0);
    starLabel.setLayoutY(70);
    starLabel.setStyle(
        "-fx-font-family: 'FontAwesome'; -fx-text-fill: #FFD700; -fx-font-size: 14px;");
    ...
    pane2.getChildren()
        .addAll(imageView, titleLabel, authorLabel, viewsLabel, ratingLabel, starLabel, eyeIcon,
            categoryLabel);
  ...
  /**
   * global handle section
   *
   * @param list           bookList
   * @param selectedButton button
   * @param mode           currentMode
   */
  ...
  private void resetCategoryButtonStyles() {
    viewAll.setStyle(
        "-fx-background-color: transparent; -fx-background-radius: 5px; -fx-cursor: hand;");
    viewBorrowed.setStyle(
        "-fx-background-color: transparent; -fx-background-radius: 5px; -fx-cursor: hand;");
    viewDelay.setStyle(
        "-fx-background-color: transparent; -fx-background-radius: 5px; -fx-cursor: hand;");
    viewPending.setStyle(
        "-fx-background-color: transparent; -fx-background-radius: 5px; -fx-cursor: hand;");
    viewReturned.setStyle(
        "-fx-background-color: transparent; -fx-background-radius: 5px; -fx-cursor: hand;");
    viewLate.setStyle(
        "-fx-background-color: transparent; -fx-background-radius: 5px; -fx-cursor: hand;");
  }
  ...
  /**
   * divide book into each page
   *
   * @param bookIssueList bookList
   * @return book each page
   */
  private ArrayList<ArrayList<BookIssue>> paginateBookIssueList(
      ArrayList<BookIssue> bookIssueList) {
    ArrayList<ArrayList<BookIssue>> pages = new ArrayList<>();
    for (int i = 0; i < bookIssueList.size(); i += ITEMS_PER_PAGE) {
      pages.add(new ArrayList<>(
          bookIssueList.subList(i, Math.min(i + ITEMS_PER_PAGE, bookIssueList.size()))));
    }
    return pages;
  }
  ...
  private void displayPage(int pageIndex) {
    ...
    for (BookIssue book : currentBooks) {
      Pane bookPane = MainGUI.currentUser.getUserType().equals("admin") ? createBookIssuePane(book)
          : createBookIssuePaneUser(book);
  ...
  public Pane createBookIssuePaneUser(BookIssue book) {
    ...
    Label bookTitleLabel = createLabel(book.getBookTitle(), 39, 14, 100);
    Label authorTileLabel = createLabel(book.getBookAuthor(), 167, 14, 150);
    Label issueDateLabel = createLabel(book.getIssueDate().toString(), 337, 14, 150);
    Label returnDateLabel = createLabel(book.getReturnDate().toString(), 510, 14, 150);
    ...
    if (book.getStatus().equalsIgnoreCase("Borrowed")) {
      ...
      returnButton.setStyle(
          "-fx-background-color: #ff4d4d; -fx-background-radius: 15px; -fx-text-fill: white; -fx-cursor: hand;");
      ...
      returnButton.setOnAction(event -> {
        ...
        sendNotification(1000, MainGUI.currentUser.getId(),
            " " + currentBook.getTitle() + " successfully !", 0);
    ...
    pane.getChildren()
        .addAll(bookTitleLabel, authorTileLabel, issueDateLabel, returnDateLabel, line);
  ...
  public Pane createBookIssuePane(BookIssue book) {
    ...
    Label userIdLabel = createLabel(String.valueOf(book.getUserId()), 36, 25, 100);
    Label userEmailLabel = createLabel(book.getUsername(), 115, 25, 150);
    Label bookTitleLabel = createLabel(book.getBookTitle(), 299, 25, 150);
    Label bookAuthorLabel = createLabel(book.getBookAuthor(), 460, 25, 150);
    Label issueDateLabel = createLabel(book.getIssueDate().toString(), 642, 25, 150);
    Label returnDateLabel = createLabel(book.getReturnDate().toString(), 845, 25, 150);
    ...
    if (book.getStatus().equalsIgnoreCase("Pending")) {
      ...
      checkButton.setStyle(
          "-fx-background-color: #62f270; -fx-background-radius: 50px; -fx-cursor: hand;");
      ...
      denyButton.setStyle(
          "-fx-background-color: #ff4d4d; -fx-background-radius: 50px; -fx-cursor: hand;");
      ...
      statusLabel.setStyle(
          "-fx-background-color: " + (book.getStatus().equalsIgnoreCase("Returned") ? "#00ff1e"
              : "#ff1e1e") + "; -fx-background-radius: 20px;");
      statusLabel.setFont(new Font("System Bold", 13));
      pane.getChildren().add(statusLabel);
    }
    ...
    pane.getChildren()
        .addAll(userIdLabel, userEmailLabel, bookTitleLabel, bookAuthorLabel, issueDateLabel,
            returnDateLabel, line);
    return pane;
  }
   ...
   * @return label
   */
  private Label createLabel(String text, double x, double y, double width) {
